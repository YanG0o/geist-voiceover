package com.kigi.baseview.theme
...
import com.kigi.baseview.AppConfig
import com.kigi.baseview.utils.DateUtils
...
object ThemeConfig {
    ...
    val sThemeType: MutableState<ThemeType> =
        mutableDataSaverStateOf(DefaultDataSaverPreferences, "theme_type", ThemeType.Default)
    val lightDarkMode: MutableState<LightDarkMode> =
        mutableDataSaverStateOf(DefaultDataSaverPreferences, "light_dark_mode", LightDarkMode.System)

    fun updateThemeType(new: ThemeType) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S && new == ThemeType.DynamicNative) {
            Log.d(TAG, "Android 12 以上才支持动态主题哦")
            return
        }

        // 如果是 FromXXX，必须 64 位才行
        if ((new is ThemeType.DynamicFromImage || new is ThemeType.StaticFromColor) && !is64Bit()) {
            Log.d(TAG, "抱歉，由于库底层限制，仅 64 位机型才支持自定义取色")
...
fun TransTheme(
    ...
    val colorScheme =
        if (AppConfig.sSpringFestivalTheme.value && DateUtils.isSpringFestival) SpringFestivalColorPalette
        else when (ThemeConfig.sThemeType.value) {
            ThemeType.StaticDefault -> if (dark) DarkColorScheme else LightColorScheme
    ...
    when (ThemeConfig.sThemeType.value) {
        ThemeType.StaticDefault, ThemeType.DynamicNative -> {
            MaterialTheme(
                colorScheme = colorScheme ?: if (dark) DarkColorScheme else LightColorScheme,
                content = mContent
            )
        }
